...
# dishes_from_DB = 0
def dishes_output(message,OUR_JSON_LIST): #вывод блюд
    for i in OUR_JSON_LIST['response']['items']:
...
def func_food(message):
    if message.text.lower() == 'стоп':
        ...
        OUR_JSON_LIST = json.dumps(requests.request(method='get',
         url='http://127.0.0.1:8000/api/recipe', data=str(ingrediends)).json()).encode('utf8').decode()
        # dishes_from_DB = json.loads() #сюда файл из базы данных, нужно допилить
        dishes_output(message,OUR_JSON_LIST) #вывод блюд
        ingrediends.clear()
        # print(a)
        return
