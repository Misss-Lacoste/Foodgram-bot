...
def func_food(message):
    if message.text.lower() == 'стоп':
        ...
        OUR_JSON_LIST = json.dumps(ingrediends, ensure_ascii=False).encode('utf8').decode()
        dishes_from_DB = requests.request(method='post', url='http://127.0.0.1:8000/api/recipe', data=OUR_JSON_LIST).json()
